...
if [[ "$PID" == "--self-test" ]]; then
  echo "phoenix-dev-discover self-test OK"
  exit 0
fi

[[ -n "$PID" ]] || fail "Usage: phoenix-dev discover <project-id>"
[[ "$PID" =~ ^[A-Za-z0-9._-]+$ ]] || fail "Invalid project id."
for c in docker jq find sort sha256sum awk sed base64; do
...
stage "Inspecting registered container $CONTAINER"
...
stage "Mapping project-associated host paths"
{
...
while IFS= read -r root; do
  [[ -n "$root" ]] || continue
  echo "$root" >> "$TMP/roots.expanded"
  case "$root" in
    /mnt/user/appdata/*/*)
      rest="${root#/mnt/user/appdata/}"
      app="${rest%%/*}"
      echo "/mnt/user/appdata/$app" >> "$TMP/roots.expanded"
      ;;
  esac
done < "$TMP/roots.raw"
sort -u "$TMP/roots.expanded" | while IFS= read -r root; do
  [[ -e "$root" ]] && echo "$root"
done > "$TMP/roots.txt"
jq -R -s 'split("\n") | map(select(length > 0))' "$TMP/roots.txt" > "$TMP/roots.json"

stage "Locating registered project logs"
: > "$TMP/logs.all"
while IFS=$'\t' read -r src dst; do
  [[ -d "$src" ]] || continue
  if [[ "$dst" == "/logs" || "$src" == */logs || "$src" == */logs/* ]]; then
    find "$src" -maxdepth 2 -type f \( -name '*.log' -o -name '*.txt' \) -print 2>/dev/null >> "$TMP/logs.all" || true
  fi
done < <(jq -r '.mounts[]? | [.source,.destination] | @tsv' "$TMP/container.json")
sort -u "$TMP/logs.all" | sed -n '1,100p' > "$TMP/logs.txt"
jq -R -s 'split("\n") | map(select(length > 0))' "$TMP/logs.txt" > "$TMP/logs.json"

stage "Inspecting project-scoped deployment artifacts"
: > "$TMP/source_candidates.all"
: > "$TMP/installer_candidates.all"
: > "$TMP/rollback_candidates.all"
while IFS= read -r root; do
  [[ -d "$root" ]] || continue
  find "$root" -maxdepth 5 -type f \
    ...
    -print 2>/dev/null >> "$TMP/source_candidates.all" || true
  find "$root" -maxdepth 5 -type f \
    \( -iname '*install*.sh' -o -iname '*update*.sh' -o -iname '*deploy*.sh' \) \
    ! -iname '*rollback*' -print 2>/dev/null >> "$TMP/installer_candidates.all" || true
  find "$root" -maxdepth 6 -type f -iname '*rollback*.sh' -print 2>/dev/null >> "$TMP/rollback_candidates.all" || true
done < "$TMP/roots.txt"

for name in source_candidates installer_candidates rollback_candidates; do
  sort -u "$TMP/$name.all" | sed -n '1,200p' > "$TMP/$name.txt"
  jq -R -s 'split("\n") | map(select(length > 0))' "$TMP/$name.txt" > "$TMP/$name.json"
done

stage "Inspecting application files baked into the image"
if docker exec "$CONTAINER" sh -lc 'true' >/dev/null 2>&1; then
  ...
  ' > "$TMP/image_source_files.all" 2>/dev/null || true
else
  : > "$TMP/image_source_files.all"
fi
sed -n '1,200p' "$TMP/image_source_files.all" > "$TMP/image_source_files.txt"
jq -R -s 'split("\n") | map(select(length > 0))' "$TMP/image_source_files.txt" > "$TMP/image_source_files.json"

stage "Locating matching Unraid template"
TEMPLATE=""
if [[ -d /boot/config/plugins/dockerMan/templates-user ]]; then
  ...
  done < <(find /boot/config/plugins/dockerMan/templates-user -maxdepth 1 -type f -name '*.xml' -print 2>/dev/null | sort)
fi
TEMPLATE_SHA=""
...
stage "Writing Phoenix project discovery state"
jq -n \
  --arg project_id "$PID" \
