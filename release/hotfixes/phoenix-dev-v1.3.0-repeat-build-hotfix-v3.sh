...
BACKUP_DIR="$BASE/releases/v1.3.0-repeat-build-hotfix-v3"
BACKUP="$BACKUP_DIR/phoenix-dev-workflow.before"
TMP="$(mktemp /tmp/phoenix-dev-workflow-hotfix-v3.XXXXXX)"
...
awk '/^checkpoint_source\(\) \{/ { exit } { print }' "$TARGET" > "$TMP.part1"
...
log "Phoenix repeat-build hotfix v3 installed"
echo "Backup: $BACKUP"
echo "Checkpoint path: validated"
